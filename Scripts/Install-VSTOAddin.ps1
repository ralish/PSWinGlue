<#
    .SYNOPSIS
    Installs a Visual Studio Tools for Office (VSTO) add-in

    .DESCRIPTION
    Launches the installation of a VSTO add-in using VSTOInstaller.

    .PARAMETER ManifestPath
    Path to the VSTO manifest for the VSTO add-in to be installed.
    ...
    .PARAMETER Silent
    Runs the VSTO installation silently.

    .EXAMPLE
    Install-VSTOAddin.ps1 -ManifestPath "https://example.s3.amazonaws.com/live/MyAddin.vsto" -Silent
    ...
    For a silent installation the certificate with which the manifest is signed must be present in the Trusted Publishers certificate store.
...
Write-Verbose -Message ('Installing VSTO add-in with arguments: {0}' -f [String]::Join(' ', $VstoArguments))
