Function Remove-AlternateDataStream {
    ...
    if ($PathItem -isnot [IO.DirectoryInfo]) {
        throw ('Expected directory but received: {0}' -f $PathItem.GetType().Name)
    }
    ...
    Get-ChildItem -Path $Path -Recurse:$Recurse -File | Remove-Item -Stream $Streams -Verbose:$VerbosePreference -ErrorAction Ignore
}
